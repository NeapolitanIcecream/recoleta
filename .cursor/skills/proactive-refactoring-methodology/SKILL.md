...
description: Evaluate whether the codebase is accumulating structural debt and decide what to refactor next with the repo's `cremona` profile. Use when the user asks for code-quality audits, refactor prioritization, architecture hotspots, code corrosion checks, or dead-code review.
---
...
2. Run `uv run cremona scan` for a whole-repo audit, or pass a narrowed scope such as `uv run cremona scan recoleta/pipeline recoleta/site.py`.
3. Generate coverage data with `uv run coverage run -m pytest` and `uv run coverage json -o coverage.json`, then pass it with `--coverage-json coverage.json` when you want full routing signals.
...
7. Only run `uv run cremona scan --coverage-json coverage.json --update-baseline` when the debt was genuinely reduced or the previous baseline schema or semantics were clearly obsolete.
