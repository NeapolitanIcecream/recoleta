...
from recoleta.models import Document, DocumentChunk, PassOutput
...
def test_ideas_stage_consumes_canonical_trend_pass_output_and_writes_projection(
    ...
    with Session(repository.engine) as session:
        ...
        assert (
            freshness["components"]["upstream_pass_output_id"]
        ...
        assert diagnostics["evidence_quality_gate"] == {
            ...
            "read_refs": [[doc_id, 0] for doc_id in sorted(evidence_doc_ids)],
        }
...
def test_ideas_stage_generates_bundle_title_from_normalized_retained_ideas(
    ...
    assert captured["summary_md"] == (
        "- **Prompt release gate** — Add a release gate before prompt rollout."
    )
...
@pytest.mark.parametrize(
    ("title_failure_mode", "output_language", "idea_title", "expected_title"),
    [
        (
            "exception",
            "English",
            "Auditable long-horizon eval workbench",
            "Auditable long-horizon eval workbench",
        ),
        (
            "invalid",
        ...
        ("exception", "zh-CN", "Ideas", "跨来源研究路径"),
        ("invalid", "zh_Hans", "Ideas", "跨来源研究路径"),
        ("exception", "中文", "Ideas", "跨来源研究路径"),
        ("invalid", "简体中文", "Ideas", "跨来源研究路径"),
    ],
)
def test_ideas_stage_uses_valid_deterministic_title_when_title_rewrite_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    title_failure_mode: str,
    output_language: str,
    idea_title: str,
    expected_title: str,
) -> None:
    ...
    monkeypatch.setenv("RECOLETA_DB_PATH", str(tmp_path / "recoleta.db"))
    monkeypatch.setenv("LLM_MODEL", "test/fake-model")
    monkeypatch.setenv("LLM_OUTPUT_LANGUAGE", output_language)
    monkeypatch.setenv("RAG_LANCEDB_DIR", str(tmp_path / "lancedb"))
    ...
    ideas_payload = _ideas_payload(
        period_start=period_start,
        period_end=period_end,
        evidence_doc_ids=evidence_doc_ids,
    )
    ideas_payload = ideas_payload.model_copy(
        update={
            "ideas": [
                ideas_payload.ideas[0].model_copy(update={"title": idea_title})
            ]
        }
    )
    monkeypatch.setattr(
    ...
    if title_failure_mode == "exception":
        bundle_title = lambda **_kwargs: (_ for _ in ()).throw(  # noqa: E731
            RuntimeError("title model timeout")
        )
    else:
        bundle_title = lambda **_kwargs: (  # noqa: E731
            "Ideas: 2026",
            {
    ...
    assert ideas_agent.bundle_title_rewrite_reason(result.title) is None
    assert result.note_path is not None
    assert result.note_path.exists()
    ...
    metric_name = (
        "pipeline.trends.pass.ideas.bundle_title_failed_total"
        if title_failure_mode == "exception"
        else "pipeline.trends.pass.ideas.bundle_title_invalid_total"
    )
    assert metric_values[metric_name] == 1.0


def test_ideas_stage_propagates_latest_suppressed_trend_without_model_call(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("PUBLISH_TARGETS", "markdown")
    ...
    stale_succeeded_id = _persist_trend_synthesis_pass_output(
        repository=repository,
        run_id="run-trend-old-success",
        granularity="day",
        period_start=period_start,
        period_end=period_end,
        payload=payload,
    )
    suppressed_upstream_id = _persist_trend_synthesis_pass_output(
        repository=repository,
        run_id="run-trend-new-suppression",
        granularity="day",
        period_start=period_start,
        period_end=period_end,
        payload=payload.model_copy(update={"clusters": []}),
        status=PassStatus.SUPPRESSED,
    )
    assert suppressed_upstream_id > stale_succeeded_id
    ...
    monkeypatch.setattr(
        ideas_agent,
        "generate_trend_ideas_payload",
        lambda **_kwargs: pytest.fail("suppressed upstream must not invoke the model"),
    )

    result = service.ideas(
        run_id="run-ideas-after-trend-suppression",
        granularity="day",
        anchor_date=date(2026, 3, 2),
    )

    assert result.status == PassStatus.SUPPRESSED.value
    assert result.upstream_pass_output_id == suppressed_upstream_id
    assert result.note_path is None
    with Session(repository.engine) as session:
        row = session.get(PassOutput, result.pass_output_id)
        assert row is not None
        diagnostics = json.loads(row.diagnostics_json or "{}")
        input_refs = json.loads(row.input_refs_json or "[]")
    assert diagnostics["upstream_suppressed"] is True
    assert diagnostics["suppression_projection_complete"] is True
    assert diagnostics["usage"]["requests"] == 0
    assert input_refs[0]["pass_output_id"] == suppressed_upstream_id
...
def test_ideas_stage_suppressed_rerun_removes_stale_file_projections_only(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("PUBLISH_TARGETS", "markdown,obsidian")
    monkeypatch.setenv("MARKDOWN_OUTPUT_DIR", str(tmp_path / "md"))
    ...
    _persist_trend_synthesis_pass_output(
        repository=repository,
        run_id="run-trend-upstream-suppressed-cleanup",
        granularity="day",
        period_start=period_start,
        period_end=period_end,
        payload=_trend_payload(
    ...
    monkeypatch.setattr(
        ideas_agent,
        "generate_trend_ideas_bundle_title",
        lambda **_kwargs: pytest.fail("bundle title generation should be skipped"),
    )

    succeeded = service.ideas(
        run_id="run-ideas-before-suppression",
        granularity="day",
        anchor_date=date(2026, 3, 2),
    )

    assert succeeded.status == PassStatus.SUCCEEDED.value
    assert succeeded.note_path is not None
    markdown_note = succeeded.note_path
    markdown_sidecar = presentation_sidecar_path(note_path=markdown_note)
    ...
    assert markdown_note.exists()
    assert markdown_sidecar.exists()
    assert obsidian_note.exists()

    with Session(repository.engine) as session:
        idea_document = session.exec(
        ...
        assert idea_document.id is not None
        idea_document_id = int(idea_document.id)
        projected_snapshot = (
            idea_document.title,
            [
                (chunk.chunk_index, chunk.kind, chunk.text, chunk.source_content_type)
                for chunk in sorted(
                    session.exec(
                        select(DocumentChunk).where(
                            DocumentChunk.doc_id == idea_document_id
                        )
                    ).all(),
    ...
    settings.publish_targets.clear()
    suppressed = service.ideas(
        run_id="run-ideas-after-suppression",
        granularity="day",
        anchor_date=date(2026, 3, 2),
    )

    assert suppressed.status == PassStatus.SUPPRESSED.value
    assert suppressed.note_path is None
    assert not markdown_sidecar.exists()
    assert not markdown_note.exists()
    assert not obsidian_note.exists()

    with Session(repository.engine) as session:
        idea_document = session.get(Document, idea_document_id)
        assert idea_document is not None
        current_snapshot = (
            idea_document.title,
            [
                (chunk.chunk_index, chunk.kind, chunk.text, chunk.source_content_type)
    ...
    assert current_snapshot == projected_snapshot
    ...
    assert (
        metric_values[
            "pipeline.trends.projection.ideas_markdown_suppressed_cleanup.emitted_total"
        ]
        == 1.0
    )
    assert (
        metric_values[
            "pipeline.trends.projection.ideas_obsidian_suppressed_cleanup.emitted_total"
        ]
        == 1.0
    )
    assert (
        "pipeline.trends.projection.ideas_documents.emitted_total" not in metric_values
    )
